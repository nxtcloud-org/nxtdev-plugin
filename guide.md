...
> This repo is **private**, so installing requires (1) access to `nxtcloud-org/nxtdev-plugin` (all org members have this) and (2) a GitHub **HTTPS credential** on your machine. The marketplace clones over HTTPS, so an SSH key alone does **not** authenticate the install.
>
> Simplest setup — log in once via `gh`:
> ```bash
> gh auth login   # choose GitHub.com → HTTPS
> ```
> On macOS with a git credential manager installed, the first install may instead pop a browser login automatically — that works too.

> **Troubleshooting — install fails with an authentication / `could not read Username` error**
> Your machine has no GitHub HTTPS credential. Run `gh auth login` (HTTPS) as above, then retry `/plugin install`. Verify access with:
> ```bash
> git ls-remote https://github.com/nxtcloud-org/nxtdev-plugin.git v1.0.1   # prints a tag hash on success
> ```
...
> **사전 조건 (사내 팀원)**
> 이 레포는 **비공개**라, 설치하려면 (1) `nxtcloud-org/nxtdev-plugin` 접근 권한(org 멤버는 모두 보유) (2) 머신에 GitHub **HTTPS 자격증명**이 필요합니다. 마켓플레이스는 HTTPS로 클론하므로, **SSH 키만으로는 설치 인증이 되지 않습니다.**
>
> 가장 간단한 설정 — `gh`로 한 번 로그인:
> ```bash
> gh auth login   # GitHub.com → HTTPS 선택
> ```
> macOS에 git credential manager가 깔려 있으면, 첫 설치 시 브라우저 로그인 창이 자동으로 떠서 처리되기도 합니다.

> **문제 해결 — 인증 오류 / `could not read Username`으로 설치 실패 시**
> 머신에 GitHub HTTPS 자격증명이 없는 경우입니다. 위처럼 `gh auth login`(HTTPS)을 한 뒤 `/plugin install`을 재시도하세요. 접근 확인:
> ```bash
> git ls-remote https://github.com/nxtcloud-org/nxtdev-plugin.git v1.0.1   # 성공 시 태그 해시 출력
> ```
